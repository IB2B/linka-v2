...
import { INSTRUCTION_FIELDS, VISUAL_PLATFORMS } from "./ai-instruction-fields";
...
export function AiInstructionsForm({ platform, data }: Props) {
  ...
  const fields = INSTRUCTION_FIELDS.filter(
    (f) => !f.visualOnly || VISUAL_PLATFORMS.has(platform),
  );
