...
export function PricingCard({ plan }: { plan: Plan }) {
  ...
  return (
    <div
      className={`relative flex flex-col gap-6 rounded-xl border p-7 ${
        hi
          ? "border-[#6D5FF9]/40 bg-white shadow-[0_24px_60px_-30px_rgba(109,95,249,0.45)] ring-1 ring-[#6D5FF9]/20"
    ...
    >
      ...
      <div>
        <div className="flex items-baseline gap-1.5">
          <span className="text-[40px] font-semibold tracking-tight text-[#0F1113]">
            {plan.price}
          </span>
        ...
        <p
          className="mt-2 text-[14px] leading-[1.5] tracking-tight text-[#525252]"
      ...
      <ul className="flex flex-col gap-2.5 border-t border-[#E5E5E5] pt-5">
        {plan.features.map((f) => (
          <li key={f} className="flex items-start gap-2 text-[13px] tracking-tight">
            <Check className="mt-0.5 size-4 shrink-0 text-[#00B67A]" />
            <span
