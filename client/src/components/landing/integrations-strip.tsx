...
export async function IntegrationsStrip() {
  ...
  return (
    <section className="relative z-10 mx-auto w-full max-w-5xl px-6 py-20">
      ...
      <div className="mx-auto mt-9 grid w-full max-w-3xl grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
        {INTEGRATIONS.map((i) => (
          <Card
            key={i.name}
            size="sm"
            className="items-center bg-white py-4 text-[#0F1113] ring-[#E5E5E5] transition hover:-translate-y-0.5 hover:shadow-[0_8px_24px_-12px_rgba(15,17,19,0.12)]"
          >
            <span
              aria-hidden
            ...
            >
              <BrandIcon id={i.id} className="size-5" />
            </span>
