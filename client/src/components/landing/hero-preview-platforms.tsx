...
export async function HeroPreviewPlatforms() {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-t border-[#F4F4F5] px-5 py-3">
      <p className="text-[10.5px] font-medium uppercase tracking-[0.14em] text-[#A3A3A3]">
        {t("topPlatforms")}
      </p>
      <div className="flex items-center gap-4">
        {SPLIT.map((p) => (
          <span key={p.slug} className="flex items-center gap-1.5">
            ...
            >
              <PlatformIcon platform={p.slug} className="size-2.5" />
            </span>
            <span className="text-[11px] tracking-tight text-[#525252]">{p.label}</span>
            <span className="text-[11px] font-medium tracking-tight text-[#0F1113]">
              {p.share}
            </span>
          </span>
        ))}
