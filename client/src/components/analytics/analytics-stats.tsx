...
export function AnalyticsStatsRow({ stats }: { stats: AnalyticsStats }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <AnalyticsStatCard label="All content" delta={stats.total} icon={FileText} />
      <AnalyticsStatCard label="Drafts" delta={stats.drafts} icon={Sparkles} />
      <AnalyticsStatCard label="Scheduled" delta={stats.scheduled} icon={CalendarClock} />
      <AnalyticsStatCard label="Published" delta={stats.posted} icon={Send} />
    </div>
  );
}
